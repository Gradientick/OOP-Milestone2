// src/components/AdminRegister.jsx (Modified)

import React, { useState, useEffect, useMemo } from 'react'; // Add useEffect
import { auth, db } from '../firebase';
import { createUserWithEmailAndPassword } from 'firebase/auth';
import { doc, setDoc } from 'firebase/firestore';
// import your employees array (replace with the actual path)

function AdminRegister() {
    const [successMessage, setSuccessMessage] = useState('');
    const [errorMessage, setErrorMessage] = useState('');
    const [registrationComplete, setRegistrationComplete] = useState(false); // Track completion
    ...
    const employees = useMemo(() => [
        ...
        },
        {
          ...
          grossSemiMonthlyRate: 26335,
          hourlyRate: 313.5119048,
          email: 'beatriz@motorph.com'
        }
      ], []); 

    useEffect(() => {
        const registerAllEmployees = async () => {
            if (registrationComplete) return; // Prevent re-running

            try {
                for (const employee of employees) {
                    // Generate email (you might have this already in your data)
                    const email = employee.email;
                    const password = 'password123'; // VERY IMPORTANT: Use a strong, temporary password.  Employees MUST change this.
                   ...
                   console.log(`User ${email} registered successfully with ID: ${user.uid}`);
                }
                setSuccessMessage('All employees registered successfully!');
    ...
    }, [registrationComplete, employees]); // Run when component mounts or employees data changes.

      if (registrationComplete) {
        return <p>Registration complete!</p>;  // Or redirect to another page
    }
